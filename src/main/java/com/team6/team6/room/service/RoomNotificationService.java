...
public class RoomNotificationService {
    ...
    public void sendExpirationWarningNotification(String roomKey) {
        ...
        messagingTemplate.convertAndSend("/topic/room/" + roomKey + "/messages", KewordChatMessage.roomExpiryWarning());
    }

    public void sendClosedNotification(String roomKey) {

        log.info("방 종료 알림 발송: {}", roomKey);
        messagingTemplate.convertAndSend("/topic/room/" + roomKey + "/messages", KewordChatMessage.roomExpired());
    }

    public void leaderRoomClosedNotification(String roomKey) {
        log.info("리더 방 종료 알림 발송: {}", roomKey);
        messagingTemplate.convertAndSend("/topic/room/" + roomKey + "/messages", KewordChatMessage.leaderRoomExpired());
    }
